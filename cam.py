...
class Main:
    def __init__(self, device):
        ...
        if not camera:
            self.cap = cv2.VideoCapture(str(Path(video).resolve().absolute()))
        ...
        while self.should_run():
            ...
            if debug:  # face
                if self.gaze is not None and self.left_bbox is not None and self.right_bbox is not None:
                    ...
                    h = np.arctan2(self.gaze[0], -self.gaze[2]) * 180 / np.pi
                    v = np.arctan2(self.gaze[1], -self.gaze[2]) * 180 / np.pi

                    yield h, v

                    cv2.putText(self.debug_frame, f'{self.gaze}',  ( 0, 50), cv2.FONT_HERSHEY_PLAIN, 5, (255, 0, 0), 1, cv2.LINE_AA)
                    cv2.putText(self.debug_frame, f'h={h:.1f}, v={v:.1f}', (0, 100), cv2.FONT_HERSHEY_PLAIN, 5, (255, 0, 0), 1, cv2.LINE_AA)
